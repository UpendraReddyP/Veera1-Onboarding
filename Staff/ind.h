...
<body>
    ...
    <script>
            ...
            employees.forEach(employee => {
                ...
                card.innerHTML = `
                    <div class="card-header">
                        <img class="profile-pic" src="http://16.171.19.248:3068/api/onboarding/${employee.id}/file/profilePic" alt="Profile Picture" onerror="this.src='https://via.placeholder.com/50'; this.alt='No Profile Picture'">
                        <div class="info-container">
        ...
        async function showEmployeeDetails(employeeId) {
            try {
                const response = await fetch(`http://16.171.19.248:3068/api/onboarding/${employeeId}`);
                if (!response.ok) {
                    throw new Error(`HTTP ${response.status}: ${await response.text()}`);
                }
                ...
                document.getElementById('modalProfilePic').src = `http://16.171.19.248:3068/api/onboarding/${employee.id}/file/profilePic`;
